...
## useState概念解释
我们第一个要学习的Hook(钩子函数)是useState，他的作用是“勾住”函数组件中自定义数据状态。  
...
答：useState能够解决类组件 **所有自定义数据变量只能存储在this.state** 的问题。

举例：若某组件需要有2个自定义数据变量name和age，那么在类组件中只能如下定义
...
2、任何你想定义的数据变量都可以单独拆分出去，独立定义，互不影响；

两段代码对比之下，你就会发现使用Hook的useState后，会让我们的定义的数据变量相对独立，清晰简单，便于管理。  
...
useState(value)函数会返回一个数组，该数组包含2个元素：第1个元素为数据对应的默认值，第2个元素为修改该数据对应的函数名称。  
...
#### 'variable'补充说明
1、variable为数据变量名，实际使用中可以修改成任意变量名，比如name、age、count等等；  
