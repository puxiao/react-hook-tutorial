...
## useMemo概念解释
我们第六个要学习的Hook(钩子函数)是useMemo，他的作用是“勾住”组件中某些处理函数的返回值，创建这些返回值对应在react原型链上的索引。当组件重新渲染时，需要再次用到这些函数返回值，此时不再重新执行一遍运算，而是直接使用之前运算过的返回值。useMemo第2个参数是保存这些处理函数的依赖，只有当处理函数依赖的数据发生改变时才会重新计算并保存一次函数返回结果。
...
1、不是所有fn(函数)都适用的，必须是该函数有返回值，即 return xx 才可以。  
2、虽然都是fn，但是函数体内代码内容却相差很大，useCallback中的fn主要用来处理各种操作事务的代码，例如修改某变量值或加载数据等。而useMemo中的fn主要用来处理各种计算事务的代码。  
3、useCallback和useMemo都是为了提升组件性能，但是他们两个的适用场景却不相同，并不是谁是谁的替代品或谁是谁的简化版。  
...
补充说明：  
1、只有当组件重新渲染时，useMemo才会起作用，因此使用useMemo并不需要子组件必须使用React.memo。  
...
useMemo(create,deps)函数通常传入2个值，第1个值为我们定义的一个“包含复杂计算且有返回值的函数”，第2个值为该处理函数中存在的依赖变量，请注意凡是处理函数中有的数据变量都需要放入deps中。如果处理函数没有任何依赖变量，可以传入一个空数组[]。
