...
## useCallback概念解释
我们第五个要学习的Hook(钩子函数)是useCallback，他的作用是“勾住”组件属性中某些处理函数，创建这些函数对应在react原型链上的索引。useCallback第2个参数是保存这些处理函数的依赖，只有当处理函数依赖的数据发生改变时才会重新修改并创建新的一份处理函数。

懵圈了没？我已经尽量总结得让你容易理解了，如果你似懂非懂没有关系，本文后面会有更加详细的阐述。
